...
bool FileCommandLoader::LoadFile(const std::string& filePath)
{
	...
	while(!inFile.eof())
	{
		...
		if((commandPos = line.find(":")) != std::string::npos)
		{
			...
			for(size_t commandIndex = 0; commandIndex < aCommands.size(); ++commandIndex)
			{
				if(commandStr == aCommands[commandIndex].command)
				{
					if(aCommands[commandIndex].commandType == COMMAND_ONE_LINE)
					{
						ParseFuncParams params;
						params.dilimitPos = dilimitPos;
						params.lineNum = 0;
						params.line = line;
						aCommands[commandIndex].parseFunc(params);
					}
					else
					{
