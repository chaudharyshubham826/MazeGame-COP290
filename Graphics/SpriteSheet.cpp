...
{
	bool loadedImage = aBMPImage.Load(App::Singleton().GetBasePath() + std::string("Assets/") + name + ".bmp");
...
{
	size_t length = aSections.size();

	for(size_t i = 0; i < length; ++i)
	{
		if(StringCompare(aSections[i].key, spriteName))
		{
			return aSections[i].sprite;
		}
	}
...
bool SpriteSheet::LoadSpriteSections(const std::string& path)
{
	...
	keyCommand.parseFunc = [&](ParseFuncParams params)
		{
			aSections.back().key = FileCommandLoader::ReadString(params);
		};
	...
	xPosCommand.parseFunc = [&](ParseFuncParams params)
		{
			aSections.back().sprite.xPos = FileCommandLoader::ReadInt(params);
		};
	...
	yPosCommand.parseFunc = [&](ParseFuncParams params)
		{
			aSections.back().sprite.yPos = FileCommandLoader::ReadInt(params);
		};
	...
	widthCommand.parseFunc = [&](ParseFuncParams params)
		{
			aSections.back().sprite.width = FileCommandLoader::ReadInt(params);
		};
	...
	heightCommand.parseFunc = [&](ParseFuncParams params)
		{
			aSections.back().sprite.height = FileCommandLoader::ReadInt(params);
		};
